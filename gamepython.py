...
class SeleccionResolucion(tk.Tk):
    def __init__(self):
        super().__init__()

        self.title("Seleccionar Resolución")
        self.geometry("300x200")
        self.eval('tk::PlaceWindow . center')

        ttk.Label(self, text="Ingrese la resolución deseada:").pack(pady=10)

        self.ancho_entry = ttk.Entry(self)
        self.alto_entry = ttk.Entry(self)

        ttk.Label(self, text="Ancho:").pack()
        self.ancho_entry.pack()

        ttk.Label(self, text="Alto:").pack()
        self.alto_entry.pack()

        ttk.Button(self, text="Aceptar", command=self.aplicar_resolucion).pack(pady=10)

    def aplicar_resolucion(self):
        try:
            nuevo_ancho = int(self.ancho_entry.get())
            nuevo_alto = int(self.alto_entry.get())

            if nuevo_ancho > 0 and nuevo_alto > 0:
                self.destroy()  # Cierra la ventana de selección de resolución
                app = AhorcadoApp(nuevo_ancho, nuevo_alto)
        except ValueError:
            tk.messagebox.showerror("Error", "Ingrese valores numéricos válidos para ancho y alto.")
...
class AhorcadoApp(tk.Tk):
    def __init__(self, ancho, alto):
        ...
        # Cargar la imagen de fondo y redimensionarla al tamaño de la ventana
        background_image = Image.open("fondo_fds.png")
        background_image = background_image.resize((ancho, alto))
...
if __name__ == "__main__":
    seleccion_resolucion = SeleccionResolucion()
    seleccion_resolucion.mainloop()
